...
const GraphPage = () => {
      ...
      .then((data) => {
        ...
        data.forEach((rating: any) => {
          ...
          if (!nodes.find((n) => n.data.id === politicianId)) {
            nodes.push({
              data: { id: politicianId, label: `Politician ${rating.politician_id}` },
              classes: "politician",
            });
          }
          ...
          // Add Edge (Rating)
          edges.push({
            data: { id: `edge-${rating.id}`, source: politicianId, target: companyId, label: `${rating.stars} ⭐` },
            classes: "rating",
          });
        });
  ...
  return (
    <>
      ...
      <div ref={cyRef} style={{ width: "100%", height: "600px", border: "1px solid black" }} />
    </>
  );
};
