...
export default function PoliticianGraph({ politicianId }: PoliticianGraphProps) {
      ...
      .then((data: Rating[]) => {
        ...
        data.forEach((rating) => {
          ...
          if (!nodes.find((n) => n.data.id === politicianNodeId)) {
            nodes.push({
              data: { id: politicianNodeId, label: `Politician ${rating.politician_id}` },
              classes: "politician",
            });
          }
  ...
  const addAdditionalPolitician = async (pol: Politician) => {
    ...
    try {
      const resRatings = await fetch(`/api/graph/ratings?politicianId=${newPolId}`);
      if (!resRatings.ok)
      ...
      if (filteredRatings.length === 0) {
        alert("This politician has no connection to the companies in the original graph.");
        return;
      }
      ...
      const similarity = Math.round((agreements / filteredRatings.length) * 100);
        ...
        {search.length > 0 && searchResults.length > 0 && (
          ...
          >
            {searchResults.map((result) => (
              ...
              >
                <strong>
                  {result.first_name} {result.last_name} &bull; {result.party?.short}
                </strong>
