    ...
    return (
        <div className={"relative mt-4"}>
            ...
            {search.length > 0 && politicians.length > 0 &&
                <div
                    className={"absolute w-full bg-white dark:bg-gray-800 shadow-lg rounded z-20 mt-2 overflow-scroll max-h-72"}>
                    {politicians.map((politician, index, array) => {
                        return (
                            <Link key={politician.uuid} href={`/politician/${politician.uuid}`}>
                                <div
                                    className={"flex p-3 w-full text-left hover:bg-gray-200 dark:hover:bg-gray-700 dark:border-gray-500 transition-colors hover:cursor-pointer " + ((index !== array.length - 1) ? "border-b" : "")}>
                                    <Image className={"rounded-full"}
                                           src={`/pol_profile_img/${politician.profile_image}`}
                                           alt={""} width={52}
                                           height={100}/>
                                    <div className={"ml-3"}>
                                        <h3 className={"text-lg font-semibold dark:text-white"}>{politician.first_name} {politician.last_name} &bull; {politician.party?.short}</h3>
                                        <p className={"dark:text-gray-300"}>{politician.occupation}</p>
                                    </div>
                                </div>
                            </Link>
                        )
