...
import {deleteUserAction, makeUserAdminAction} from "@/app/(default)/admin/userAdminAction";
...
export function UserList({defaultUsers}: { defaultUsers: User[] }) {
    ...
    async function makeUserAdmin(userId: number) {
        ...
        setUsers(users.map(u => ({...u, isAdmin: u.id === userId ? !u.isAdmin : u.isAdmin})));
    }
    ...
    return (
        <>
            {error && (
                <Toast message={"Error: " + error} type={"error"} onClose={() => setError(null)}/>
            )}
            ...
            <table className={"w-full"}>
                ...
                {users.map(user => (
                    <tr key={user.id} className={"hover:bg-gray-100"}>
                        ...
                        <td>
                            <button
                                className={"bg-indigo-800 hover:bg-indigo-700 transition-colors text-white rounded px-3 py-1.5 ml-5"}
